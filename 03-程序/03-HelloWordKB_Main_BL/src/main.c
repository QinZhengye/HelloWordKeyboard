...
const char * const _usb_strings[5] = {
	"HelloWord", // iManufacturer
	"HelloWord Keyboard Bootloader", // iProduct
...
usbdfu_control_request(struct usb_setup_data *req,
	...
	case DFU_DNLOAD:
		if ((len == NULL) || (*len == 0)) {
			...
			my_memcpy(prog.buf, usbd_control_buffer, prog.len);
	...
	case DFU_UPLOAD:
		...
		if (!req->wValue) {
			...
			*len = 0;
			...
			const uint32_t end_addr   = 0x08000000 + (        FLASH_SIZE_KB*1024);
			if (baseaddr >= start_addr && baseaddr + DFU_TRANSFER_SIZE <= end_addr) {
				my_memcpy(usbd_control_buffer, (void*)baseaddr, DFU_TRANSFER_SIZE);
