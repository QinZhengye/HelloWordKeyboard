...
extern void *my_memcpy(void * dst, const void * src, size_t count);
...
static enum usbd_request_return_codes usb_standard_get_descriptor() {
	...
	switch (descr_type) {
	case USB_DT_DEVICE:
		my_memcpy(usbd_control_buffer, &dev_desc, sizeof(dev_desc));
		datasize = sizeof(dev_desc);
		return USBD_REQ_HANDLED;
	case USB_DT_CONFIGURATION:
		my_memcpy(usbd_control_buffer, &config_desc, sizeof(config_desc));
