...
float Motor::GetEstimateAngle()
{
    ...
    state.rawAngle = (float) (encoder->countDirection) * (encoder->GetFullAngle() - zeroAngleOffset);
    state.estAngle = config.lpfAngle(state.rawAngle);

    return state.estAngle;
}

void Motor::ApplyZeroAngle()
{
    if (encoder)
        encoder->ResetRotationCount();

    float magLapAngle = encoder->GetLapAngle();
    zeroAngleOffset = magLapAngle;
    target = 0.0f;
    setPointAngle = 0.0f;
    estimateAngle = 0.0f;
    state.rawAngle = 0.0f;
    state.estAngle = 0.0f;
    printf("zeroAngleOffset=%.3f\r\n", zeroAngleOffset);
}

float Motor::GetEstimateLapAngle()
{
    if (!encoder) return 0.0f;
    return config.lpfLapAngle(encoder->GetLapAngle());
}
...
bool Motor::AlignSensor()
{
    if (encoder->countDirection == EncoderBase::UNKNOWN)
    {
        ...
        if (midAngle == endAngle)   // // TODO 考虑微小波动
        {
