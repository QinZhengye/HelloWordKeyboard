...
{
    static const float clkBase = 72000000.0f;
    float psc = 0.5;
    float arr;
    ...
    {
        psc *= 2;
        arr = clkBase / psc / (float) _freq;
    } while (arr > 65535);

    if (htim->Instance == TIM7 || htim->Instance == TIM13 || htim->Instance == TIM14) // APB1 @72MHz
    {
        PSC = (uint16_t) round((double) psc);
        ARR = (uint16_t) (clkBase / (float) _freq / psc);
    } else if (htim->Instance == TIM10 || htim->Instance == TIM11) // APB2 @144MHz
    {
